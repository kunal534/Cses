...
int tryPath(int pathIdx, int curR, int curC) {
    ...
    // if left and right are traversed and upper and down are yet to traverse then pruning is done as ounce we go up then no chance for down
	if ((onPath[curR][curC - 1] && onPath[curR][curC + 1]) &&
	    (!onPath[curR - 1][curC] && !onPath[curR + 1][curC]))
		return 0;

    // if up and down are traversed and both left and right are free to tranverse then if one is selected the other will be neglected henced pruned
	...
	onPath[curR][curC] = false;// if failed backtrack
	return ret;
}
...
int main() {
	...
	for (int i = 0; i < PATH_LEN; i++) {
		...
		else p[i] = 4;  // chance to explore
	}

    // marking the boundary
	for (int i = 0; i < GRID_SIZE; i++) {
		onPath[0][i] = true;// upper row
		onPath[8][i] = true;// lower row
		onPath[i][0] = true;// left col
		onPath[i][8] = true;// right col
	}
